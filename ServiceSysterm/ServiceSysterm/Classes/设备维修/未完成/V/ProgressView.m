...
#define DefaultLightColor [UIColor blueColor] //点亮颜色值
...
//    CGFloat margin = 0;
//    CGFloat containVW = maxTitleLabelW + TitleLabelMargin;
//    debugLog(@"= = =%f",containVW);
//    CGFloat containVX = (kScreenWidth - containVW * titlesArr.count - margin * (titlesArr.count - 1)) / 2;
    CGFloat progressX = 0;
    CGFloat progressY = 0;
    CGFloat progressW = 0;
    for (NSUInteger i = 0; i < titlesArr.count; i++) {
        UIView *containV = [[UIView alloc]init];
        containV.frame = CGRectMake(5 + (kScreenWidth-20)/titlesArr.count*i, 0, (kScreenWidth-20)/titlesArr.count, self.frame.size.height);
        ...
        titleLabel.frame = CGRectMake(0, CGRectGetMaxY(circleLabel.frame) + SubVerticalMargin , containV.frame.size.width, titleLabel.frame.size.height);
