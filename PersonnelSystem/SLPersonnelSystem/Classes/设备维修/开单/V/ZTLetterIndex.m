...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.itemSize = CGSizeMake(16.0f, 16.0f);
...
- (void)setupItems
{
    ...
    [self.dataArray enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        item.frame = CGRectMake((self.bounds.size.width - weakSelf.itemSize.width)/2, weakSelf.itemSize.height * idx + (self->_slidr.bounds.size.height - weakSelf.itemSize.height), weakSelf.itemSize.width, weakSelf.itemSize.height);
        item.backgroundColor = [UIColor clearColor];
        item.text = [NSString stringWithFormat:@"%@",obj];
        item.font = weakSelf.textFont;
        item.textAlignment = NSTextAlignmentCenter;
        item.textColor = weakSelf.textColor;
        item.tag = 999 + idx;
        ...
        longPressGR = [[UILongPressGestureRecognizer alloc] initWithTarget:weakSelf action:@selector(touchLong:)];
        longPressGR.allowableMovement=NO;
        longPressGR.minimumPressDuration = 0.2;
        
        singleTapGR = [[UITapGestureRecognizer alloc] initWithTarget:weakSelf action:@selector(touchDown:)];
...
- (void)selectIndex:(NSInteger)index;
{
    ...
    [UIView animateWithDuration:kAnimationDuration
                     animations:^{
                         self->_slidr.frame = CGRectMake(self->_slidr.frame.origin.x, index*self.itemSize.height, self->_slidr.bounds.size.width, self->_slidr.bounds.size.height);
                     }
